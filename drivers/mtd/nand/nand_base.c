...
//#define CFG_NAND_PAGES_IN_BLOCK		    128
...
/* Nand flash oob definition for 2Kb page size with 8_bit ECC */
////Samsung K9K8G08U0E forlinx.bs
#if 0
                            ...
                            struct mtd_oob_ops *ops)
{
    ...
    // printf("from=%d,realpage=%d,page=%d\n",from,realpage,page);
                           ...
                           const uint8_t *buf, int page, int cached, int raw)
{
...
#endif
    {
        ...
        char * ppp = chip->oob_poi;
        int iii = mtd->oobsize >> 3;

        while (iii--) {
            printf( "\t%02x %02x %02x %02x %02x %02x %02x %02x\n",
                    ppp[0], ppp[1], ppp[2], ppp[3], ppp[4], ppp[5], ppp[6], ppp[7]);
            ppp += 8;
        }
#endif


        /*forlinx add codes for clear oob data,it is safty.
          Reset oob to FF  when page==128 Page,
          or leavings pages have ecc data in 12--23 positions in oob region ,so it is wrong.
        */
        if(page==CFG_NAND_PAGES_IN_BLOCK)
        {
           memset(chip->oob_poi, 0xff, mtd->oobsize);
                              ...
                              struct mtd_oob_ops *ops)
{
    ...
    //forlinx add
    memset(chip->oob_poi,0xFF, len);



#if 0

    printf("before nand_fill_oob...........\n");


    char * pp = chip->oob_poi;
    int ii = len >> 3;
                             ...
                             struct mtd_oob_ops *ops)
{
    ...
    printf("X210 ------before nand_do_write_ops  ..........\n");


    char * pp = buf;
    int ii = mtd->writesize >> 3;
    ...
    printf("X210 ------before write_page  ..........\n");


    char * ppp = buf;
    int iii = mtd->writesize >> 3;
    ...
    *maf_id = chip->read_byte(mtd);
    ...
    if (!type->pagesize) {
	    ...
	    //Samsung K9K8G08U0E      size=1GB  type=SLC  Page=2K OOB=64bytes
            ...
            busw = 0;
#ifdef FORLINX_DEBUG
            printf("Samsung K9K8G08U0E No  No Calc pagesize, blocksize, erasesize,  use ids table .............\n");
#endif
        }
