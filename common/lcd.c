...
void bitmap_plot (int x, int y)
{
	...
	if ((panel_info.vl_bpix) < 12) 
	{
	    ...
	    //if (reg == BOOT_NAND)
		{
			...
			fb16 = (ushort *)(lcd_base);
			for (i=0; i<panel_info.vl_row/*BMP_LOGO_HEIGHT*/; ++i) {
				for (j=0; j<panel_info.vl_col/*BMP_LOGO_WIDTH*/; j++) {
					//fb16[j] = bmap16[j];
					//fb16[j]= 0xf800;//red=1111100000000000
					//fb16[j]= 0x7e0;//green=111111100000	
					fb16[j]= 0x1f;//blue=11111
					}
				bmap16 += panel_info.vl_col;//BMP_LOGO_WIDTH;
				fb16 += panel_info.vl_col;
			}	
		}//end ifif (reg == BOOT_NAND)
	}
...
#if 1
	memset(lcdbase,0x00,panel_info.vl_row*panel_info.vl_row*2);
	printf("fb_size: %d\n",fb_size);
	printf("fb_size*2: %d\n",fb_size*2);
	printf("panel_info.vl_row: %d\n",panel_info.vl_row);
	printf("panel_info.vl_col: %d\n",panel_info.vl_col);
