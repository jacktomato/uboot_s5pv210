...
#include <bmp_logo.h>
//#define VIDEO_LOGO_WIDTH	800//BMP_LOGO_WIDTH
//#define VIDEO_LOGO_HEIGHT	480//BMP_LOGO_HEIGHT forlinx.zcg
...
int video_display_bitmap (ulong bmp_image, int x, int y)
{
	...
	if (!((bmp->header.signature[0] == 'B') &&
		...
		if (gunzip(dst, CFG_VIDEO_LOGO_MAX_SIZE, uboot_logo_bmp_gz, &len) != 0) {
		//forlinx.bs
			printf ("Error: no valid bmp or bmp.gz image at %lx\n", bmp_image);
			free(dst);
			return 1;
...
void logo_plot (void *screen, int width, int x, int y)
{
	...
	unsigned char *dest = (unsigned char *)screen +
			...
			while (xcount--) {
				...
				case GDF_15BIT_555RGB:
					...
					*(unsigned short *) dest =
					...
					break;
				case GDF_16BIT_565RGB:
					*(unsigned short *) dest = *source;//forlinx.zcg
		...
		writel(0xe, GPD0DAT);//GPD0DAT GPD0DAT[3:2],[0]=1,GPD0DAT[1]=0
...
//	if (reg == BOOT_NAND)
	...
	if (reg == BOOT_NAND)//bluesky
	{
		logo_plot (video_fb_address, VIDEO_COLS, 0, 0);
	}else if(reg==BOOT_MMCSD)//bluesky
	{
		sprintf (info, " %s", &version_string);
		video_drawstring (VIDEO_INFO_X, VIDEO_INFO_Y, (uchar *)info);  //forlinx.zcg
		memset(info,0x0,128);//
		strncpy(info," Flashing the nandflash,four leds will turn on after the completion of all,please wait...",90);
		video_drawstring (VIDEO_INFO_X,VIDEO_INFO_Y + 1 * VIDEO_FONT_HEIGHT,(uchar *)info);
		//turn on the backlight forlinx.bs
		//forlinx.zcg  pwm1 backlight
		writel(0x1010, GPD0CON);//GPD0CON GPD0CON[3]，[0]:output GPD0CON[2:1]:input  
		writel(0x15, GPD0PUD);//GPD0PUD GPD0PUD[3]:Pull-up/ down disabled,GPD0PUD[2:0]:Pull-down enabled
		writel(0xc0, GPD0DRV);//GPD0DRV GPD0DRV[3]:4x,GPD0DRV[2:0]:1x
...
static int video_init (void)
{
    ...
    if ((pGD = video_hw_init ()) == NULL)//call  /driver/video/s5pv210_fb.c
		return -1;
	...
	//video_display_bitmap((ulong)0, 0, 0);
	
#ifdef CONFIG_VIDEO_LOGO
	/* Plot the logo and get start point of console */
	PRINTD ("Video: Drawing the logo ...\n");
    video_console_address = video_logo (); //call
#else
